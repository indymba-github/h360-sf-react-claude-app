...
export default function PresetPreview({ preset, size = "mini" }: PresetPreviewProps) {
  ...
  const navItem = (active: boolean): React.CSSProperties => ({
    height: 3 * scale,
    borderRadius: 1,
    background: active ? `${palette.accent}33` : `${palette.text}11`,
  });
  ...
  const kpiCard = (i: number): React.CSSProperties => ({
    background: i === 0 ? `${palette.accent}18` : `${palette.text}08`,
    borderTop: i === 0 ? `1.5px solid ${palette.accent}` : `1.5px solid ${palette.text}22`,
  ...
  return (
    <div style={root}>
      {/* Header bar — uses headerBg / headerFg from palette */}
      <div style={headerBar}>
        <div style={logoBox}>
          {logoDataUrl ? (
      ...
      {/* Body — uses palette.paper bg and palette.text */}
      <div style={body}>
        {/* Sidebar nav */}
        <div style={sidebar}>
